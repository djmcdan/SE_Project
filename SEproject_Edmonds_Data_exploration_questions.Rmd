...
+ 2. Questions 
    Q1. Dplyr.
        ...
        Where the new vehicles really new based on the amount of milage ?
...
zipcode_tran <- new_bought_cat %>% select(zip) %>% distinct()
data(zipcode)

new_bought_latlon = merge(new_bought_cat,zipcode, by.x='zip', by.y='zip') %>% select(visitor_key, date_sold,model_bought,make_bought,model_year_bought,time_hours_cat,city,state,latitude,longitude)
